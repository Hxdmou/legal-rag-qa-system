    ...
    for shape in slide.shapes:
        if shape.has_table:
            ...
            data = [
                ...
                ["应用层", "高", "多模态融合", "95%", "企业服务", "<10ms", "中规模", "GPT 5.0+DeepSeek V5.0", "高"],
    ...
    for shape in slide.shapes:
        if shape.has_table:
            ...
            data = [
                ...
                ["WAF防火墙", "Web攻击防护", "Cloudflare/Akamai", "极高", "边缘", "<5ms", "高", "互联网", "10"],
                ["OAuth 2.0", "身份认证", "Keycloak/Okta", "高", "集中", "<15ms", "中", "企业级", "9"],
                ["mTLS", "双向认证", "Istio/Linkerd", "极高", "服务网格", "<20ms", "中高", "金融", "10"],
                ["RBAC/ABAC", "权限控制", "OPA/Casbin", "高", "集中", "<5ms", "低", "企业级", "9"],
                ["DDoS防护", "流量清洗", "Cloudflare/AWS Shield", "极高", "边缘", "<1ms", "高", "互联网", "10"],
                ["数据加密", "静态+传输", "AES-256/SM4", "极高", "全链路", "<3ms", "低", "金融医疗", "10"],
                ["审计日志", "操作追踪", "ELK/Splunk", "高", "集中", "<100ms", "中", "企业级", "9"],
                ["威胁检测", "实时告警", "SIEM/XDR", "高", "集中", "<50ms", "高", "安全运营", "9"],
                ["密钥管理", "密钥轮换", "HashiCorp Vault", "极高", "集中", "<10ms", "中", "企业级", "10"],
                ["数据脱敏", "敏感数据保护", "Privitar/Databricks", "高", "数据层", "<50ms", "中", "金融医疗", "9"],
                ["访问审计", "权限审计", "Azure AD/Ping", "高", "集中", "<20ms", "中", "企业级", "8"],
                ["Agent安全", "智能体认证", "A2A-Security", "高", "分布式", "<30ms", "中", "AI系统", "9"],
                ["AI伦理", "偏见检测", "IBM AI Fairness", "中", "AI层", "<100ms", "中", "AI应用", "8"],
                ["安全审计", "合规检查", "SOC2/ISO27001", "极高", "定期", "-", "高", "金融医疗", "10"],
                ["安全培训", "员工教育", "KnowBe4", "中", "定期", "-", "低", "全员", "7"],
                ["安全测试", "渗透测试", "Burp Suite/Nessus", "高", "定期", "-", "中", "开发阶段", "9"],
                ["零信任架构", "永不信任", "BeyondCorp", "极高", "全架构", "<50ms", "高", "企业级", "10"],
                ["隐私计算", "安全多方计算", "MPC/FHE", "极高", "数据层", "<500ms", "极高", "金融", "9"],
                ["SOC2合规", "安全认证", "第三方审计", "极高", "定期", "-", "高", "SaaS", "10"],
            ]
    ...
    for shape in slide.shapes:
        if shape.has_table:
            ...
            data = [
                ...
                ["健康检查", "故障自愈", "K8s Probe", "低", "低", "99.9%", "中", "9", "低"],
                ...
                ["服务网格", "流量管理", "Istio/Linkerd", "中高", "高", "99.99%", "高", "9", "中"],
                ["自动化运维", "DevOps", "Jenkins/GitLab", "中", "中", "99.9%", "高", "9", "低"],
                ["日志管理", "可观测性", "ELK/Loki", "中", "中", "-", "高", "9", "低"],
                ["指标监控", "性能追踪", "Prometheus", "中", "低", "-", "高", "10", "低"],
                ["分布式追踪", "链路分析", "Jaeger/Zipkin", "中", "中", "-", "高", "9", "低"],
                ["配置管理", "动态配置", "ConfigMap/etcd", "低", "低", "99.99%", "高", "10", "低"],
                ["存储编排", "持久化", "CSI/Rook", "中", "中", "99.99%", "高", "9", "中"],
                ["网络策略", "安全隔离", "Calico/Cilium", "中", "中", "99.99%", "高", "9", "中"],
                ["自动化测试", "CI/CD", "Selenium/Cypress", "中", "中", "-", "高", "9", "低"],
                ["容量规划", "资源预测", "AI预测", "中", "中", "-", "极高", "8", "低"],
                ["安全扫描", "漏洞检测", "Trivy/Snyk", "低", "低", "-", "高", "10", "低"],
                ["Agent部署", "智能体管理", "A2A-Orchestrator", "中", "中", "99.9%", "高", "9", "中"],
                ["Agent监控", "智能体追踪", "LangSmith", "中", "低", "-", "高", "9", "低"],
    ...
    for shape in slide.shapes:
        if shape.has_table:
            ...
            data = [
                ...
                ["消息批量", "Kafka批量", "吞吐+100%", "低", "低", "1个月", "大数据", "9", "P1"],
                ["缓存策略", "Redis+CDN", "延迟-60%", "低", "中", "2个月", "高并发", "10", "P0"],
                ["并发处理", "AsyncIO+ThreadPool", "吞吐+200%", "中", "低", "1个月", "API服务", "9", "P1"],
                ["序列化", "Protobuf/FlatBuffers", "性能+50%", "中", "中", "2个月", "微服务", "8", "P2"],
                ["CDN加速", "Cloudflare/Akamai", "延迟-70%", "低", "高", "1个月", "互联网", "10", "P0"],
                ["数据库优化", "索引+读写分离", "查询+300%", "中", "中", "3个月", "大数据", "9", "P1"],
                ["代码优化", "JIT+编译优化", "性能+30%", "高", "低", "2个月", "核心服务", "8", "P2"],
                ["资源调度", "K8s调度优化", "利用率+40%", "中", "中", "2个月", "云原生", "9", "P1"],
                ["连接池", "复用+限流", "性能+50%", "低", "低", "1个月", "数据库", "9", "P1"],
                ["内存优化", "GC调优+对象池", "内存-30%", "高", "低", "2个月", "Java服务", "8", "P2"],
                ["异步IO", "EventLoop+协程", "并发+500%", "中", "低", "2个月", "高并发", "9", "P1"],
                ["Agent优化", "LLM缓存+微调", "成本-60%", "高", "中", "3个月", "AI服务", "10", "P0"],
                ["AI推理优化", "量化+蒸馏", "速度+200%", "高", "中", "3个月", "AI服务", "10", "P0"],
                ["索引优化", "倒排+向量索引", "查询+1000%", "中", "中", "2个月", "搜索服务", "10", "P0"],
                ["负载均衡", "智能路由", "可用性+99.99%", "低", "中", "1个月", "大规模", "9", "P0"],
                ["预热机制", "缓存预热", "首屏-50%", "低", "低", "1个月", "电商", "8", "P2"],
                ["熔断降级", "Hystrix/Sentinel", "可用性+99.9%", "低", "低", "1个月", "微服务", "10", "P0"],
                ["读写优化", "分离+缓存", "性能+200%", "中", "中", "2个月", "数据库", "9", "P1"],
                ["边缘计算", "EdgeAI", "延迟-80%", "中", "高", "3个月", "IoT", "9", "P1"],
                ["GPU加速", "CUDA+TensorRT", "AI+1000%", "高", "极高", "2个月", "AI训练", "10", "P0"],
                ["压缩优化", "GZIP/Brotli", "带宽-60%", "低", "低", "1个月", "API服务", "8", "P2"],
    ...
    for shape in slide.shapes:
        if shape.has_table:
            ...
            data = [
                ...
                ["智能推荐", "电商", "DeepSeek V5.0+实时", "转化+35%,GMV+25%", "500万", "6个月", "中", "9", "高"],
                ["智能教育", "教育", "GPT 5.0+个性化", "效果+40%,辍学率-20%", "300万", "8个月", "中", "9", "高"],
                ["智能制造", "制造", "AI质检+预测维护", "次品率-50%,停机-40%", "800万", "12个月", "中高", "9", "中"],
                ["智慧城市", "政府", "多智能体+数据融合", "效率+30%,成本-25%", "2000万", "24个月", "高", "8", "中"],
                ["Agent协作", "AI", "A2A协议+多智能体", "效率+100%,能力+200%", "500万", "6个月", "中", "10", "高"],
                ["Agent生态", "平台", "智能体市场+API", "生态价值+300%", "1000万", "18个月", "高", "9", "中"],
                ["Agent创新", "科研", "AI+Agent", "发现速度+500%", "800万", "12个月", "高", "9", "低"],
                ["综合案例", "全行业", "A2A+AI", "全面提升", "-", "-", "-", "10", "-"],
            ]
...
def add_slide8(prs):
    ...
    data = [
        ...
        ["AMQP", "<30ms", "TLS", "高", "中等", "高", "中", "成熟", "7.5"],
        ["对比结论", "A2A最优", "A2A最优", "A2A最优", "A2A最优", "A2A最优", "中等", "成长中", "A2A领先"],
    ]
    
    for i in range(len(data)):
        for j in range(len(data[i])):
            fill_cell(table.cell(i, j), data[i][j], 12 if i == 0 else 9, i == 0 or j == 0)
    
    print("✅ 项目8已添加")

def add_slide9(prs):
    """添加项目9：市场预测（2026-2035）"""
    slide_layout = prs.slide_layouts[1]
    slide = prs.slides.add_slide(slide_layout)
    
    # 添加标题
    title = slide.shapes.title
    title.text = "项目9：市场预测（2026-2035年）"
    ...
    data = [
        ...
        ["合计", "288300亿", "CAGR 52%", "-", "62800亿", "-", "-", "AI时代"],
    ]
...
def main():
    input_path = r"f:\个人作品\legal-rag-qa-system\docs\presentations\A2A_ENTERPRISE_PPT_V12_完整版.pptx"
    output_path = r"f:\个人作品\legal-rag-qa-system\docs\presentations\A2A_ENTERPRISE_PPT_V12_完整修复版.pptx"
    ...
    # 更新项目2-7
    if len(prs.slides) > 1:
        update_slide2(prs.slides[1])
    if len(prs.slides) > 2:
        update_slide3(prs.slides[2])
    if len(prs.slides) > 3:
        update_slide4(prs.slides[3])
    if len(prs.slides) > 4:
        update_slide5(prs.slides[4])
    if len(prs.slides) > 5:
        update_slide6(prs.slides[5])
    if len(prs.slides) > 6:
        update_slide7(prs.slides[6])
    
    # 删除最后3张幻灯片（相关实践、使用说明、THANK YOU）
