...
def fix_project3_complete(prs):
    ...
    if len(prs.slides) > 2:
        ...
        for shape in slide.shapes:
            if shape.has_table:
                table = shape.table
                print(f"📌 当前表格: {len(table.rows)}行 × {len(table.columns)}列")
                ...
                if len(table.rows) < len(data):
                    print(f"⚠️ 表格行数不足！需要{len(data)}行，当前只有{len(table.rows)}行")
                    print("⚠️ PPT表格行数限制，可能无法添加L10")
                else:
                    # 填充所有数据
