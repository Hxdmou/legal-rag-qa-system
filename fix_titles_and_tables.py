            ...
            if title_shape:
                ...
                for paragraph in title_shape.text_frame.paragraphs:
                    for run in paragraph.runs:
                        ...
                        run.font.color.rgb = RGBColor(255, 255, 255)
                        run.font.name = '微软雅黑'
            print(f"✅ 幻灯片{i+1}标题更新为: {titles[i]}")

def update_project2(slide):
    """更新项目2：价值维度"""
...
def main():
    input_path = r"f:\个人作品\legal-rag-qa-system\docs\presentations\A2A_ENTERPRISE_PPT_V12_完整修复版.pptx"
    output_path = r"f:\个人作品\legal-rag-qa-system\docs\presentations\A2A_ENTERPRISE_PPT_V12_最终版.pptx"
    
    print("🔄 正在修复所有项目...")
    prs = Presentation(input_path)
    
    # 更新标题
    update_slide_titles(prs)
    
    # 更新所有项目表格
    if len(prs.slides) > 1:
        update_project2(prs.slides[1])
        print("✅ 项目2表格已更新")
    if len(prs.slides) > 2:
        update_project3(prs.slides[2])
        print("✅ 项目3表格已更新")
    if len(prs.slides) > 3:
        update_project4(prs.slides[3])
        print("✅ 项目4表格已更新")
    if len(prs.slides) > 4:
        update_project5(prs.slides[4])
        print("✅ 项目5表格已更新")
    if len(prs.slides) > 5:
        update_project6(prs.slides[5])
        print("✅ 项目6表格已更新")
    if len(prs.slides) > 6:
        update_project7(prs.slides[6])
        print("✅ 项目7表格已更新")
    if len(prs.slides) > 7:
        update_project8(prs.slides[7])
        print("✅ 项目8表格已更新")
    if len(prs.slides) > 8:
        update_project9(prs.slides[8])
        print("✅ 项目9表格已更新")
    if len(prs.slides) > 9:
        update_project10(prs.slides[9])
        print("✅ 项目10表格已更新")
