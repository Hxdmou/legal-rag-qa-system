...
SYSTEM_NAME = "legal"
DEFAULT_INDEX_DIR = "legal_faiss_index"
...
def interactive():
    ...
    st.markdown("""
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        .feature-title { color: #ec407a; font-size: 1.15rem; font-weight: 600; margin-bottom: 0.5rem; }
        ...
        .source-card { background: #f8f9fa; border-left: 3px solid #ec407a; padding: 0.75rem; margin: 0.5rem 0; border-radius: 0 8px 8px 0; font-size: 0.85rem; }
        </style>
    ...
    st.markdown("""
        ...
        本系统提供的信息仅供参考，不构成法律意见。遇到具体法律问题请咨询专业律师。
        </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with st.expander("⚙️ 参数设置", expanded=True):
            ...
            st.session_state.temperature = st.slider("🌡️ 温度参数", 0.0, 1.0, st.session_state.temperature, 0.1, help="值越高回答越多样化")
            st.session_state.top_k = st.slider("📊 检索数量", 1, 10, st.session_state.top_k, 1, help="每次检索返回的文档数量")
            st.session_state.retrieval_mode = st.selectbox("🔍 检索模式", ["hybrid", "vector", "bm25"], index=["hybrid", "vector", "bm25"].index(st.session_state.retrieval_mode), help="hybrid: 混合检索")
            st.session_state.streaming_enabled = st.checkbox("🔄 启用流式输出", st.session_state.streaming_enabled, help="开启后回答会逐步显示")

        with st.expander("📁 文档上传", expanded=True):
            st.markdown("""<div class="sidebar-section"><h3>上传文档</h3></div>""", unsafe_allow_html=True)
            uploaded_files = st.file_uploader("选择文件", type=Config.SUPPORTED_FILE_TYPES, accept_multiple_files=True, help=f"支持格式: {', '.join(Config.SUPPORTED_FILE_TYPES)}")
            if uploaded_files:
        ...
        with st.expander("📂 本地文件", expanded=False):
            ...
            local_path = st.text_input("文件或文件夹路径", placeholder="C:/Documents/", help="支持单个文件或整个文件夹")
