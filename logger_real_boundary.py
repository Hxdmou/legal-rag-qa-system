...
def generate_boundary_report(report_filename, log_filename,
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        f.write("=== PyBullet 真实物理参数边界迁移验证报告 ===\n\n")
        ...
        if pass_rate >= 90:
            ...
            f.write(f"     建议质量边界: ±{min(mass_max*100 if mass_max else 0, 20):.0f}%\n")
            f.write(f"     建议摩擦边界: {friction_max:.4f}\n")
            f.write(f"     建议延迟边界: {delay_max} 步\n")
        elif pass_rate >= 70:
        ...
        f.write("\n  💡 迁移建议:\n")
        if pass_rate >= 90:
            f.write("     - 当前控制策略已具备迁移至真实部署的条件\n")
            f.write("     - 建议进入第二阶段：部署适配\n")
        else:
            f.write("     - 建议在仿真中增加更多物理因素深化训练\n")
            f.write("     - 建议扩大域随机化范围\n")
