...
def interactive():
    ...
    st.markdown("""
        ...
        * { font-family: 'Inter', sans-serif; }
        .main-header { background: linear-gradient(135deg, #66bb6a 0%, #43a047 100%); padding: 2.5rem; border-radius: 16px; margin-bottom: 2rem; box-shadow: 0 10px 40px rgba(102, 187, 106, 0.3); }
        .main-header h1 { color: white; font-size: 2.8rem; font-weight: 700; margin-bottom: 0.5rem; }
        .main-header p { color: rgba(255, 255, 255, 0.9); font-size: 1.2rem; font-weight: 300; }
        .feature-card { background: linear-gradient(145deg, #ffffff, #e8f5e9); border: 1px solid #c8e6c9; border-radius: 12px; padding: 1.2rem; margin-bottom: 1rem; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
        .feature-card:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(102, 187, 106, 0.15); border-color: #66bb6a; }
        .feature-title { color: #43a047; font-size: 1.15rem; font-weight: 600; margin-bottom: 0.5rem; }
        .disclaimer-card { background: linear-gradient(145deg, #fff3e0, #ffe0b2); border: 1px solid #ffcc80; border-radius: 12px; padding: 1.2rem; margin-bottom: 1.5rem; }
        ...
        .sidebar-section h3 { font-size: 1rem; font-weight: 600; color: #555; margin-bottom: 0.75rem; padding-bottom: 0.5rem; border-bottom: 1px solid #e0e0e0; }
        .source-card { background: #f8f9fa; border-left: 3px solid #66bb6a; padding: 0.75rem; margin: 0.5rem 0; border-radius: 0 8px 8px 0; font-size: 0.85rem; }
        </style>
    """, unsafe_allow_html=True)

    st.markdown(f"""<div class="main-header"><h1>{config['icon']} {config['name']}</h1><p>{config['description']} | 版本: {Config.VERSION}</p></div>""", unsafe_allow_html=True)

    st.markdown("""<div class="disclaimer-card"><h4>⚠️ 医疗免责声明</h4><p style="font-size: 0.85rem; color: #555; margin: 0;">本系统提供的信息仅供参考，不构成医疗建议。遇到健康问题请咨询专业医生。</p></div>""", unsafe_allow_html=True)

    session_defaults = {
        'messages': [], 'vector_store': None, 'current_files': [], 'engine_initialized': False,
        'show_history': False, 'show_batch': False, 'show_backup': False, 'show_docs': False,
        'retrieval_mode': "hybrid", 'temperature': 0.7, 'top_k': 5,
        'index_load_status': "未加载", 'streaming_enabled': True
    }
                    ...
                    with st.expander("📑 查看引用来源"):
                        for i, src in enumerate(message["sources"][:3], 1):
                            if isinstance(src, tuple) and len(src) >= 2:
                                doc, score = src
                                score_pct = 1 / (1 + score)
                                st.markdown(f"""<div class="source-card"><strong>来源 {i}</strong> (相关度: {score_pct:.1%})<br/>{doc.page_content[:200]}...</div>""", unsafe_allow_html=True)
            ...
            if not st.session_state.engine_initialized:
                ...
                with st.chat_message("assistant", avatar=config["icon"]):
                    with st.spinner("🤔 正在思考中..."):
                        try:
                            chain = llm_chain(st.session_state.vector_store, temperature=st.session_state.temperature, top_k=st.session_state.top_k, retrieval_mode=st.session_state.retrieval_mode)
    ...
    with col2:
        ...
        for title, desc in features:
            st.markdown(f"""<div class="feature-card"><div class="feature-title">{title}</div><p style="color: #666; font-size: 0.85rem; margin: 0;">{desc}</p></div>""", unsafe_allow_html=True)

        st.markdown("---")
        st.subheader("📊 系统状态")
        status_color = "status-warning" if not st.session_state.engine_initialized else "status-ready"
        status_text = "⚠️ 等待初始化" if not st.session_state.engine_initialized else "✅ 系统就绪"
        ...
        with col_btn5:
            if st.button("📤 导出对话", use_container_width=True):
                if st.session_state.messages:
                    output_path = export_conversation_to_markdown({'system': config['name'], 'timestamp': st.session_state.messages[0]['content'] if st.session_state.messages else 'N/A', 'messages': st.session_state.messages})
    ...
    if st.session_state.show_batch:
        st.markdown("---")
        create_batch_interface(lambda q: llm_chain(st.session_state.vector_store, temperature=st.session_state.temperature, top_k=st.session_state.top_k).invoke(q))
    ...
    st.markdown("---")
    st.markdown(f"""<div style="text-align: center; color: #888; font-size: 0.85rem;"><strong>{config['icon']} {config['name']}</strong> | 版本 {Config.VERSION}</div>""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with st.expander("⚙️ 参数设置", expanded=True):
            ...
            st.session_state.temperature = st.slider("🌡️ 温度参数", 0.0, 1.0, st.session_state.temperature, 0.1)
            st.session_state.top_k = st.slider("📊 检索数量", 1, 10, st.session_state.top_k, 1)
            st.session_state.retrieval_mode = st.selectbox("🔍 检索模式", ["hybrid", "vector", "bm25"], index=["hybrid", "vector", "bm25"].index(st.session_state.retrieval_mode))
            st.session_state.streaming_enabled = st.checkbox("🔄 启用流式输出", st.session_state.streaming_enabled)

        with st.expander("📁 文档上传", expanded=True):
            st.markdown("""<div class="sidebar-section"><h3>上传文档</h3></div>""", unsafe_allow_html=True)
            uploaded_files = st.file_uploader("选择文件", type=Config.SUPPORTED_FILE_TYPES, accept_multiple_files=True)
            if uploaded_files:
                ...
                if st.button("🚀 处理并添加", use_container_width=True):
                    with st.spinner("正在处理文档..."):
                        file_paths = [save_uploaded_file(f) for f in uploaded_files if save_uploaded_file(f)]
                        if file_paths:
        ...
        with st.expander("📂 本地文件", expanded=False):
            ...
            local_path = st.text_input("文件或文件夹路径", placeholder="C:/Documents/")
            if local_path and st.button("📂 加载本地文件", use_container_width=True):
                with st.spinner("正在加载..."):
                    if os.path.isfile(local_path):
                        success, msg = process_files([local_path])
                    elif os.path.isdir(local_path):
                        file_paths = [os.path.join(root, f) for root, _, files in os.walk(local_path) for f in files if f.endswith(tuple(Config.SUPPORTED_FILE_TYPES))]
                        success, msg = process_files(file_paths) if file_paths else (False, "文件夹中没有支持的文件")
                    else:
