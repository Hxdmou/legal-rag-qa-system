...
def interactive():
    ...
    with col1:
        st.markdown("""
            ...
            <strong>💡 重要提示</strong>
            <p>本系统提供的法律信息仅供参考，不能替代专业法律咨询。如有法律问题，请咨询专业律师。</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
        ...
        <strong>{config['icon']} {config['name']}</strong> | 版本 {Config.VERSION} | 仅供法律参考，不替代专业法律咨询
        </div>
    """, unsafe_allow_html=True)
            ...
            with st.spinner("⚖️ 正在处理法律文档..."):
                ...
                if file_paths:
                    try:
                        ...
                        st.session_state.engine_initialized = True
                        st.success(f"🎉 已添加 {len(docs)} 份法律文档")
                    except Exception as e:
