"""
 PyBullet 真实物理参数边界迁移验证（修正版）
 修正内容：
   1. 末端执行器索引：panda_link8（法兰盘），与基线测试一致
   2. 起始位姿：Panda 直立位姿
   3. 控制力：120.0
 目标：验证控制策略在物理参数偏移范围内是否仍能保持稳定
 """
...
from config_real_boundary_fixed import (
    USE_KUKA,
    TARGET_END_POS,
    MASS_OFFSET_RANGE,
    JOINT_DAMPING_RANGE,
...
from logger_real_boundary_fixed import generate_boundary_report
...
generate_boundary_report(
    report_filename="F:/个人作品/具身智能/boundary_fixed_report.txt",
    log_filename="F:/个人作品/具身智能/boundary_fixed_log.csv",
...
print("[BOUNDARY] 报告已生成: F:/个人作品/具身智能/boundary_fixed_report.txt")
