...
def move_to_position(target_pos, steps=MOVE_SPEED, friction_coeff=0.0):
    ...
    for idx, joint_idx in enumerate(joint_indices):
        p.setJointMotorControl2(robot_id, joint_idx, p.POSITION_CONTROL,
                               targetPosition=target_joints[idx], force=FORCE)
...
def converge_to_target(target_pos, friction_coeff=0.0, max_iter=50):
    """末端收敛迭代：持续修正直到误差小于阈值"""
...
def execute_task(target_pos, delay_steps, friction_coeff):
    ...
    for target_point in trajectory:
        ...
        move_to_position(delayed_target, MOVE_SPEED, friction_coeff)

    # 末端收敛迭代：关键优化
    converge_to_target(target_pos, friction_coeff)
...
print(f"[BOUNDARY] 控制力: {FORCE}, 收敛步数: {CONVERGENCE_STEPS}")
print(f"[BOUNDARY] 收敛阈值: {CONVERGENCE_THRESHOLD*1000:.0f}mm\n")
...
generate_boundary_report(
    report_filename="F:/个人作品/具身智能/boundary_fixed_v3_report.txt",
    log_filename="F:/个人作品/具身智能/boundary_fixed_v3_log.csv",
...
print("[BOUNDARY] 报告已生成: F:/个人作品/具身智能/boundary_fixed_v3_report.txt")
