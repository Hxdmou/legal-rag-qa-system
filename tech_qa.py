...
# 对话历史保存路径
CHAT_HISTORY_FILE = "chat_history_tech.json"
...
def interactive():
    ...
    st.title("💻 IT技术问答系统")
    st.markdown("程序员的智能助手，支持上传技术文档、API文档、编程教程等资料")
    ...
    with st.sidebar:
        ...
        local_file_path = st.text_input(
            "文件路径",
            placeholder=r"例如: C:\Users\Documents\API文档.pdf",
            help="输入本地文件的完整路径"
        )

        if uploaded_files or local_file_path:
            with st.spinner("正在处理技术文档并建立索引..."):
                file_paths = []
                if uploaded_files:
    ...
    if prompt := st.chat_input("输入您的技术问题..."):
